    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-0.5">Guarniciones, extras, toppings y opciones configurables por platillo</p>
        </div>
        <button onClick={startCreateGroup}
          className="flex items-center gap-2 px-4 py-2 bg-amber-500 text-[var(--text-primary)] rounded-xl text-sm font-bold hover:bg-amber-600 transition-colors">
      ...
      {/* Group Form */}
      {isEditingGroup && (
        <div className="bg-[var(--bg-surface)] border border-amber-500/30 rounded-2xl p-5">
          <h3 className="text-[var(--text-primary)] font-bold mb-4">{editingGroup ? 'Editar grupo' : 'Nuevo grupo de modificadores'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              <label className="block text-xs text-[var(--text-secondary)] mb-1">Nombre del grupo *</label>
              <input
            ...
            <div>
              <label className="block text-xs text-[var(--text-secondary)] mb-1">Selección mínima</label>
              <input
                type="number" min="0"
            ...
            <div>
              <label className="block text-xs text-[var(--text-secondary)] mb-1">Selección máxima</label>
              <input
                type="number" min="1"
            ...
            <div className="sm:col-span-2 flex items-center gap-3">
              ...
              <span className="text-sm text-[var(--text-secondary)]">Selección obligatoria (el cliente no puede omitir)</span>
            </div>
          </div>
          <div className="flex gap-2 mt-4">
            ...
            <button onClick={() => { setEditingGroup(null); setIsCreatingGroup(false); }}
              className="flex items-center gap-2 px-4 py-2 bg-slate-600 text-[var(--text-secondary)] rounded-xl text-sm hover:bg-slate-500 transition-colors">
      ...
      {groups.length === 0 ? (
        ...
        <div className="space-y-3">
          {groups.map(group => (
            <div key={group.id} className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl overflow-hidden">
              {/* Group Header */}
              <div className="flex items-center gap-3 p-4">
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-bold text-[var(--text-primary)] text-sm">{group.name}</span>
                      {group.is_required && (
                    ...
                    <p className="text-xs mt-0.5">
                      {group.options.length === 0 ? (
                        ...
                        <span className="text-[var(--text-secondary)]">{group.options.length} opción{group.options.length !== 1 ? 'es' : ''} · {group.assignedProducts?.length ?? 0} platillo{(group.assignedProducts?.length ?? 0) !== 1 ? 's' : ''} asignado{(group.assignedProducts?.length ?? 0) !== 1 ? 's' : ''}</span>
                      )}
                    </p>
                  </div>
                ...
                <button onClick={() => startEditGroup(group)}
                  className="p-2 text-[var(--text-secondary)] hover:text-amber-400 hover:bg-amber-500/10 rounded-lg transition-colors">
                  <Pencil size={14} />
                </button>
                ...
                <div className="border-t border-[var(--border)] p-4 space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">Opciones</h4>
                      <button onClick={() => startCreateOption(group.id)}
                        className="flex items-center gap-1 px-2 py-1 bg-[var(--bg-surface)] text-[var(--text-secondary)] rounded-lg text-xs hover:bg-amber-500/20 hover:text-amber-400 transition-colors">
                    ...
                    {editingOption?.groupId === group.id && (
                      <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl p-3 mb-3">
                        <div className="grid grid-cols-2 gap-2">
                          <div className="col-span-2">
                            <label className="block text-[11px] text-[var(--text-secondary)] mb-1">Nombre *</label>
                            <input
                              value={optionForm.name}
                              onChange={e => setOptionForm({ ...optionForm, name: e.target.value })}
                              placeholder="Ej: Papas fritas, Arroz, Vegetales..."
                              className="w-full px-2 py-1.5 bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg text-[var(--text-primary)] text-xs focus:ring-1 focus:ring-amber-500/50 focus:outline-none"
                            />
                          </div>
                          <div>
                            <label className="block text-[11px] text-[var(--text-secondary)] mb-1">Tipo de precio</label>
                            <select
                          ...
                          <div className={showPriceDeltaField ? '' : 'flex items-end'}>
                            <label className="flex items-center gap-2 cursor-pointer mt-1">
                              ...
                              <span className="text-[11px] text-[var(--text-secondary)]">Disponible</span>
                            </label>
                          </div>
                        </div>
                        <div className="flex gap-2 mt-2">
                          <button onClick={handleSaveOption}
                            className="flex items-center gap-1 px-3 py-1.5 bg-amber-500 text-[var(--text-primary)] rounded-lg text-xs font-bold hover:bg-amber-600 transition-colors">
                            <Save size={11} /><span>Guardar</span>
                          </button>
                          <button onClick={() => setEditingOption(null)}
                            className="flex items-center gap-1 px-3 py-1.5 bg-slate-600 text-[var(--text-secondary)] rounded-lg text-xs hover:bg-slate-500 transition-colors">
                    ...
                    {/* Options List */}
                    {group.options.length === 0 ? (
                      <p className="text-xs text-[var(--text-secondary)] italic py-2">Sin opciones. Agrega la primera opción.</p>
                    ) : (
                      <div className="space-y-1.5">
                        {group.options.map(opt => {
                          ...
                          return (
                            <div key={opt.id} className="flex items-center gap-2 px-3 py-2 bg-[var(--bg-surface)] rounded-xl">
                              ...
                              <button onClick={() => startEditOption(group.id, opt)}
                                className="p-1.5 text-[var(--text-secondary)] hover:text-amber-400 hover:bg-amber-500/10 rounded-lg transition-colors">
                                <Pencil size={12} />
                              </button>
                  ...
                  {/* Product Assignment */}
                  <div>
                    <h4 className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-2">Platillos asignados</h4>
                    <p className="text-[11px] text-[var(--text-secondary)] mb-2">Selecciona los platillos que deben mostrar este grupo de modificadores:</p>
                          ...
                          <button
                            ...
                            className={`flex items-center gap-2 px-3 py-2 rounded-xl text-left text-xs transition-all border ${
                              isAssigned
                                ...
                                : 'bg-[var(--bg-surface)] border-[var(--border)] text-[var(--text-secondary)] hover:border-slate-500/50'
                            }`}
                          >
                            <div className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 ${isAssigned ? 'bg-amber-500' : 'bg-slate-600'}`}>
                              {isAssigned && <Check size={10} className="text-[var(--text-primary)]" />}
                            </div>
