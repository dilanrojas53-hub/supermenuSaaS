/**
 * PhoneLoginSheet — v4.0
 * Flujo: teléfono → login/registro con contraseña → passkey opcional.
 * Sin OTP/SMS. Fallback siempre disponible: número + contraseña.
 *
 * Fix v4.0: layout para teclado virtual en Android/iOS
 *  - Cambiado de "bottom sheet" a "modal centrado con scroll"
 *  - El modal usa position:fixed con top/bottom en lugar de bottom:0
 *  - Al aparecer el teclado, el navegador reduce el viewport y el modal
 *    se reposiciona automáticamente dentro del área visible
 *  - Sin autoFocus para evitar que el teclado se abra antes de que el
 *    modal esté visible (el focus se aplica manualmente con delay)
 *  - El contenido hace scroll interno si es más alto que el área disponible
 */
...
export default function PhoneLoginSheet({
  ...
  // Focus manual con delay para que el modal esté visible antes de que aparezca el teclado
  useEffect(() => {
      ...
      {isOpen && (
        <>
          ...
          {/*
            ...
            Estrategia para teclado virtual en Android:
            - Usamos `position: fixed` con `top: 0, bottom: 0` y flex para centrar
            - El contenedor exterior ocupa todo el viewport visible
            - Cuando el teclado aparece, Android reduce el viewport (resize)
              y el modal se reposiciona automáticamente en el espacio disponible
            - overflow-y-auto en el contenido permite scroll si el formulario
              es más alto que el espacio disponible
          */}
          <div
            className="fixed inset-0 z-[310] flex items-start justify-center pointer-events-none"
            style={{ paddingTop: 'max(16px, env(safe-area-inset-top))', paddingBottom: 'max(16px, env(safe-area-inset-bottom))' }}
          >
            <motion.div
              className="w-full max-w-sm mx-4 rounded-3xl overflow-y-auto pointer-events-auto"
              style={{
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 'register' && (
                    <motion.div key="register"
                      ...
                      <p className="text-sm mb-4" style={{ color: textColor, opacity: 0.55 }}>
