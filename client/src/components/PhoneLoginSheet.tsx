/**
 * PhoneLoginSheet — v3.1
 * Flujo: teléfono → login/registro con contraseña → passkey opcional.
 * Sin OTP/SMS. Fallback siempre disponible: número + contraseña.
 *
 * Fix v3.1: layout móvil con teclado virtual
 *  - Eliminado autoFocus (causaba que el teclado se abriera antes de la animación)
 *  - Cambiado maxHeight de 90vh → 90dvh (dvh se recalcula cuando aparece el teclado)
 *  - El sheet ahora usa position fixed con bottom:0 y el contenido hace scroll interno
 *  - Se agrega paddingBottom extra para que el botón no quede tapado por el teclado
 */
import { useState, useRef, useEffect } from 'react';
import { X, Phone, ArrowRight, Loader2, User, Calendar, Lock, Eye, EyeOff, Mail, Fingerprint, Shield } from 'lucide-react';
...
export default function PhoneLoginSheet({
  ...
  // Focus manual después de que la animación de entrada termine (evita que el teclado
  // se abra antes de que el sheet esté visible, lo que causaba pantalla negra)
  useEffect(() => {
    if (isOpen && step === 'phone') {
      const timer = setTimeout(() => {
        phoneRef.current?.focus();
      }, 350); // esperar a que termine la animación spring
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Sheet
            - Usamos dvh en lugar de vh: dvh se recalcula cuando aparece el teclado virtual,
              vh no. Esto evita que el contenido quede tapado por el teclado.
            - overflow-y-auto permite scroll interno si el contenido es más alto que el sheet.
            - El sheet siempre queda anclado al bottom; el teclado lo empuja hacia arriba
              gracias a que el navegador ajusta el viewport con dvh.
          */}
          <motion.div
            className="fixed bottom-0 left-0 right-0 z-[310] rounded-t-3xl overflow-y-auto"
            style={{
                  ...
                  <motion.div key="phone"
                    ...
                    <div className="flex gap-2 mb-3">
                      ...
                      <input
                        ...
                        // Sin autoFocus — el focus se aplica manualmente con delay
                        // para evitar que el teclado se abra antes de que el sheet esté visible
                    ...
                    <p className="text-center text-[11px] mt-3 mb-2" style={{ color: textColor, opacity: 0.35 }}>
                ...
                {step === 'login' && (
                  <motion.div key="login"
                    ...
                    <button onClick={() => { setAuthStep('phone_input' as any); setError(''); setPassword(''); }}
                      className="w-full text-sm py-2 mt-2 mb-2" style={{ color: textColor, opacity: 0.5 }}>
                      ← Cambiar número
                    </button>
                  </motion.div>
                )}
                    ...
                    {isWebAuthnSupported() ? (
                      <>
                        ...
                        <button onClick={() => { setAuthStep('logged_in'); onClose(); }}
                          className="w-full py-2 text-sm mb-2" style={{ color: textColor, opacity: 0.45 }}>
                          Ahora no
                        </button>
                      </>
                    ) : (
                      <>
                        ...
                        <button onClick={() => { setAuthStep('logged_in'); onClose(); }}
                          className="w-full py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-transform mb-2"
                          style={btnPrimary}>
                          Continuar
                        </button>
                      </>
